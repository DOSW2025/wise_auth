...
describe('GestionUsuariosService', () => {
  ...
  describe('findAllPaginated', () => {
    it('should return paginated users with default values', async () => {
      ...
      const result = await service.findAllWithFilters(paginationDto as any);

      expect((prismaService as any).usuario.findMany).toHaveBeenCalledWith({
        skip: 0,
        take: 10,
        include: {
      ...
      expect((prismaService as any).usuario.count).toHaveBeenCalled();
    ...
    it('should calculate correct skip value for page 2', async () => {
      ...
      await service.findAllWithFilters(paginationDto as any);
  ...
  describe('changeRole', () => {
    it('should change user role successfully', async () => {
      ...
      expect((prismaService as any).rol.findUnique).toHaveBeenCalledWith({ where: { id: 2 } });
    ...
    it('should throw NotFoundException when role does not exist', async () => {
      ...
      expect((prismaService as any).usuario.update).not.toHaveBeenCalled();
    });
    ...
    it('should verify role exists before updating user', async () => {
      ...
      expect((prismaService as any).rol.findUnique).toHaveBeenCalled();
      expect((prismaService as any).usuario.update).toHaveBeenCalled();
    });
  ...
  describe('changeStatus', () => {
    it('should change user status successfully', async () => {
      ...
      expect((prismaService as any).estadoUsuario.findUnique).toHaveBeenCalledWith({ where: { id: 2 } });
      expect((prismaService as any).usuario.update).toHaveBeenCalledWith({
        where: { id: userId },
        data: { estadoId: 2 },
        include: {
    ...
    it('should verify status exists before updating user', async () => {
      ...
      expect((prismaService as any).estadoUsuario.findUnique).toHaveBeenCalled();
      expect((prismaService as any).usuario.update).toHaveBeenCalled();
    });
